---
name: cash-flow-builder-ap
description: "Builds the accounts payable module of a board-room and PE-diligence grade monthly direct-method cash flow forecast in Excel — a consolidated payables roll forward splitting the opening balance into significantly aged and existing pools, new purchase disbursements net of holdback, holdback accrual and release, and payment prioritization under a cash constraint. Interviews the modeler for every required input, classifies source files by the columns they carry rather than their names, and separates observed payment behaviour from stated terms so the current stretch is visible. Use whenever the user asks to build the A/P piece of a cash flow forecast, an A/P or vendor disbursement forecast, a rolling A/P cash flow model, a holdback or retainage schedule, or a payment prioritization plan. For a combined model spanning A/R, A/P, payroll, capex and/or debt, use `master-cash-flow-builder` instead, which orchestrates this skill as one of its modules. REQUIRES financial-modeling-foundation — load and follow it first, and ask the user to enable it if unavailable."
---

# Cash Flow Forecast — A/P Module
...
Builds the accounts payable half of a monthly direct-method cash flow forecast in Excel. Trade payables only — payroll, taxes, debt service and capex are separate disbursement lines and are not forecast here.

This module shares the header and footer spine with the A/R module (`cash-flow-builder-ar`). Where both are being built, build the spine once and let both read off it.
...
Ask before looking at any file, as a single block. Where the monthly A/R module already exists for this build, inherit items 1–3 rather than asking twice.
...
> 5. **Payment run cadence.** How often does a run actually go out — weekly, twice monthly, monthly, continuous? Fixed day? This mostly matters where runs are *less* frequent than monthly — weekly, bi-weekly, and monthly runs all land inside their month automatically at this periodicity.
...
**Item 5 matters less often at monthly grain than it would at finer granularity.** Where payment runs are less frequent than monthly — quarterly, semi-annual, or a handful of fixed dates a year — some months carry no disbursement at all, and a model that spreads payments evenly shows liquidity the company does not have in the months between runs. Weekly, bi-weekly, and monthly runs don't create this problem at monthly granularity — they're all at or finer than the model's own period, so every run's disbursement simply lands inside its month.
...
**Where the purchase forecast's periodicity doesn't already match the model's monthly periods, ask how to convert it — do not choose silently.** A quarterly or annual figure spread evenly assumes purchasing is uniform within the coarser period, which it rarely is. A weekly or daily source needs the reverse treatment, summed up into months. Offer even spread, working-day weighting, or a historical monthly pattern from the payment history, and record the answer in the notes as an assumption.
...
|---|---|---|---|
| Opening balance, aged/existing split | Open payables position, split on due date vs. Month 1 start | Summary aging, past-due column | Modeler input — the model cannot be built without this |
...
| Catchup profile | Payment history — how past-due balances have actually cleared | Modeler's monthly percentages | Modeler input, named as placeholder |
| Existing pool run-off | Open position due dates | Dated forward at stated terms | Modeler's monthly percentages |
...
| Holdback release | Modeler input, by period, in every case — discretionary | — | — |
...
| Monthly purchases | Purchase / COGS forecast by period | Modeler input | — |
...
> "For the part of your opening balance that isn't yet due: your aging gives me a due date on every open invoice, so I can pay it invoice by invoice, dated forward on the same convention the new-purchase gather uses. The alternative is a percentage profile — one row of monthly percentages applied to that pool. Invoice-level is more accurate and reconciles to the ledger line by line. The profile is coarser, but it is a single row a director can flex in front of a board. Which do you want? With no preference I'll build invoice-level and carry the implied percentages beneath it as an output."
...
> "Three ways to run the payments. **To behaviour** — I pay each vendor the way the company has actually been paying it, which shows what happens if nothing changes. **To terms** — I pay everything as it falls due, which shows what normalising would cost. **To constraint** — you give me a minimum cash balance to hold, and available cash each month sets what gets paid, with everything unpaid rolling into arrears. The first is the base case, the second is what vendors expect, the third is what you need if cash is tight. I can build the first two side by side — the gap between them is the funding requirement."
...
Identical to the monthly A/R module. Where both sit in one workbook the conventions are already set — do not restate them and do not diverge.

- Roboto throughout, size 11 in the forecast. **B2** entity name, Roboto 14 bold — **linked to `Macro_Assumptions`** (green font), not typed. **B3** the forecast title, built from the period count so it cannot drift (`="`&periods&`-month cash flow forecast"`). **B4** the reporting units, **linked to `Macro_Assumptions`**. Foundation defaults to actual dollars absent a stated preference.
...
*Shared with the monthly A/R module. Where it exists, read off it — do not rebuild it.*

**Build the opening column first**, per Foundation. Here it is **column F**; forecast periods begin in **column G**. It carries Month Number `0`, Month Beginning `= EDATE(forecast start date, -1)`, Projection Period - Date `= the Month Beginning cell above`, and two opening balances in the roll forward (below). Every other row in it is blank. Foundation's three rules apply: never summed into a total, never matched by a gather, visually distinct.

Header labels in **B6:B8**, values from column F:
...
| Month Beginning | The monthly start date, formulaic, driven by the forecast start date input on `Macro_Assumptions` via `EDATE` — never by adding a fixed day count | `#DAF2D0` | Black `#000000` |

Never white font on `#DAF2D0` — unreadable.

**No separate "Month" row, and no Month Beginning/Month Ending flag pair in the footer.** A finer-than-monthly spine would need a header row deriving which calendar month each period falls into, plus two footer rows flagging the first and last period of each month — machinery that exists solely to locate monthly events inside a finer-than-monthly spine. None of it has a purpose here: every column already *is* a month, so holdback accrual (below) reads the period directly rather than summarising off a flag.
...
| Transaction Adjustment | `=prior period projection period - ROUND((prior period projection period - current period projection period)/2,0)` | Places the assumed average transaction date inside the period rather than on its boundary |
...
**Why the snap matters.** `EOMONTH(d,0)+1` advances any date forward to the first day of the *following* month — every date inside a given month, including the 1st of that month itself, resolves to the same target. Because every Month Beginning is the first of a month, payment dates land exactly on header dates and the `SUMIFS` gathers match.

**Where payment runs are less frequent than monthly** — quarterly, semi-annual, or a few fixed dates a year — add a run-flag row beneath the spine: `1` on run periods, `0` otherwise, and advance the payment date to the next flagged period. Weekly, bi-weekly, and monthly runs need no such flag; they're all at or finer than the model's own grain, so every run's disbursement already lands inside its month without one.

**Twelve months is a common default, not a constant.** Foundation requires the horizon to roll forward and extend from a `Macro_Assumptions` input, never by rebuilding. Every period-driven row — the Month Number spine, Catchup, the Existing profile, the holdback release row — sizes off that input. Read "the final month" throughout this skill as "the final forecast period."

- **In Claude for Excel**, drive the spine with `SEQUENCE` off the period count, and combine periods and any total column in a single `HSTACK` so the range resizes as one unit. Generate each period's Month Beginning date with `EDATE`, never by adding a fixed day count. Cumulative Holdbacks is a stateful roll-forward and is a legitimate `SCAN`/`REDUCE` candidate — pair it with a plain-English note saying what accumulates and what releases it.
...
**Three pools, three mechanics.** The opening balance splits on due date against the Month 1 start:

| Pool | Definition | Paid by |
|---|---|---|
| **Significantly Aged** | Open items **already past due** at the Month 1 start | Catchup profile. Always modeler-driven |
| **Existing** | Open items **not yet due** at the Month 1 start | Method A or Method B per Fork 1 |
...
- **Monthly Holdbacks** is the amount withheld in each period, accrued directly off that period's purchases — `= holdback % × this period's purchases`. No flag or sub-period summarisation is needed; each column already is a month.
- **Cumulative Holdbacks** rolls forward: `prior period cumulative + holdback withheld this period − holdback released this period`. Open it at the balance from the holdback record.
- **Payments on Holdbacks** is the release, taken as **modeler input by period** — a horizontal row of release amounts, one column per forecast period.
...
- Lay Catchup out **horizontally** on `AP_Assumptions`: a formulaic row of month numbers sized to the period count, with input percentages beneath. Each is a share of the **opening significantly aged balance**.
- Pull it onto the forecast sheet with `INDEX` against the Catchup cell range, indexed on this column's Month Number. **Use a direct range reference or a `LET` label — never a Name Manager range**, per Foundation.
- **The percentages need not sum to 100%.** Where they do not, the shortfall is past-due balance still outstanding at the final forecast month — show it as an explicit residual rather than leaving it implicit in the ending balance.
- Default starting point where the modeler has no view: **60 / 40** across Months 1–2. Name it as a placeholder.
- **Under pay-to-terms**, Catchup clears the pool in Month 1 by definition — everything past due is already due. Show what that costs; it is usually the largest single month in the forecast and the reason a facility is needed.
...
```
Payments of Significantly Aged
  =-(opening significantly aged balance)*(current period catchup %)
```
...
```
=EOMONTH(due date + current stretch,0)+1
```

The stretch is zero under pay-to-terms and the observed figure under pay-to-behaviour. **Using `due date + stretch` raw, without the snap, misaligns the pool against new purchases by up to a month.**
...
Carry an explicit total for items falling due **beyond the final forecast month**, and derive the implied monthly percentages as an output row beneath the payments line, black font, no fill.

**Method B — percentage profile.** Lay the profile out horizontally, month numbers sized to the period count with input percentages beneath, each a share of the **opening existing balance**. Pull it on with `INDEX` against that range on Month Number. Where the percentages fall short of 100%, show the residual explicitly.

```
Payments of Existing
  =-(opening existing balance)*(current period profile %)
```
...
Where payment runs are less frequent than monthly, gather to the next flagged run period rather than the raw payment period.

**Committed spend from open POs and GRNI** enters Purchases in the period it is expected to be invoiced, then flows through this gather like any other purchase. Carry the expected invoice period as an input, and tie total conversions back to the PO file.
...
- The vendor spend forecast must tie to the P&L purchase line **at the finer of the two periodicities, not just in total** — every month where the P&L is monthly, every quarterly or annual subtotal where the P&L is coarser than monthly. Carry the P&L line and a variance row beneath, and require zero variance at that periodicity.
...
```
Opening A/P balance
  — Significantly aged (past due at Month 1 start)
  — Existing (not yet due at Month 1 start)
...
Committed spend not yet invoiced, by expected invoice period
Other disbursements per period (non-trade)
Payment run cadence and run-flags
```
...
| Minimum cash floor, by period | **Pay-to-constraint only** |
| Vendor tier and tier payment rule | **Pay-to-constraint only** |

**Monthly Holdbacks is calculated, not input** — it is holdback % applied directly to each period's purchases. Carry it as a formula row, not on the input list.
...
- Vendor spend forecast ties to the P&L purchase line **at the finer of the two periodicities** — every month where the P&L is monthly, every quarterly or annual subtotal where the P&L is coarser than monthly. Never only in total.
